...
def karber(
    ...
    max_animals: int,
    died: list[int],
    dose_coefficient: int = 10,
    number_of_doses: int = 8,
) -> float:
    """
    ...
    if len(died) != number_of_doses:
        raise ValueError(
            "The number of doses must equal the number of experiments.",
        )
    for i in died:
        if i > max_dose:
            raise ValueError(
                "The number of deceased animals should be"
                " less than the number of animals in each group.",
            )
    proportion_died = [died[i] / max_animals for i in range(number_of_doses)]
    ld50 = 10 ** (
        math.log(max_dose, 10)
        - math.log(dose_coefficient, 10) * (sum(proportion_died) - 0.5)
    )
    return ld50
